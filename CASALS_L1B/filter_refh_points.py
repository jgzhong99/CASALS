...
def compute_cell_median_mad(
    ...
    # Robust integer key; iy max is small, but avoid 2D array allocation.
    nx = int(ix.max() + 1)
...
def open3d_statistical_outlier_mask(
    ...
    # Use centered coordinates for numerical conditioning in Open3D.
    pts = np.column_stack([
...
def label_noise(data: RefhPointData, proj: ProjectedData, cfg: Config) -> FilterResult:
    ...
    if cfg.use_snr_amp_noise_label:
        amp_threshold = float(np.nanpercentile(data.refh_amp, cfg.amp_low_percentile))
        thresholds.update({
            "snr_hard_min": float(cfg.snr_hard_min),
            "snr_soft_min": float(cfg.snr_soft_min),
            "amp_low_percentile": float(cfg.amp_low_percentile),
            "amp_low_threshold": amp_threshold,
        })
        low_snr_hard = data.refh_snr < cfg.snr_hard_min
        low_amp = data.refh_amp < amp_threshold
        low_snr_soft = data.refh_snr < cfg.snr_soft_min
    ...
    # refh threshold logic if present.
    if cfg.use_refh_threshold_if_available and "refh_thres" in data.optional:
        thres = np.asarray(data.optional["refh_thres"], dtype=np.float64)
        valid = np.isfinite(thres)
        below = valid & ((data.refh_amp - thres) <= float(cfg.refh_threshold_margin_min))
        ...
        thresholds["refh_thres_available"] = "refh_thres" in data.optional

    # Optional error fields.
    if cfg.use_error_fields_if_available:
        if cfg.max_refh_error_m is not None and "refh_error" in data.optional:
            err = np.asarray(data.optional["refh_error"], dtype=np.float64)
        ...
        if cfg.max_refh_horizontal_error_deg is not None:
            ...
            for name in ("refh_longitude_error", "refh_latitude_error"):
                if name in data.optional:
                    err = np.asarray(data.optional[name], dtype=np.float64)
    ...
    # Global Z percentile guard, for quick inspection only.
    if cfg.use_global_z_percentile_guard:
        zlo, zhi = np.nanpercentile(data.z_refh, [cfg.z_low_percentile, cfg.z_high_percentile])
        bad = (data.z_refh < zlo) | (data.z_refh > zhi)
    ...
    # Open3D SOR.
    sor_mask, sor_info = open3d_statistical_outlier_mask(proj.easting, proj.northing, data.z_refh, cfg)
    if np.any(sor_mask):

...
1. `notebooks/01_l1b_structure_and_refh.ipynb`
2. `notebooks/01b_l1b_sweep_waveform_matrix.ipynb`
3. `extract_waveform_features.py`
4. `notebooks/02_l1b_waveform_feature_extraction.ipynb`
5. `notebooks/03_l1b_waveform_features_vs_refh_quality.ipynb`

Outputs:
- `outputs/extract_waveform_features/<h5_stem>/component_table.parquet`
- `outputs/extract_waveform_features/<h5_stem>/pulse_summary.parquet`
- `outputs/extract_waveform_features/<h5_stem>/sweep_summary.csv`
- `outputs/extract_waveform_features/<h5_stem>/diagnostic_figures/*.png`
- `outputs/extract_waveform_features/<h5_stem>/run_metadata.json`

Interpretation rules:
- Candidate component count is not a true return count.
- Prominent components are filtered waveform features, not official returns.
- Tentative height offsets derived from range-window bookkeeping are not georeferenced secondary returns.
- Waveform features may support downstream refh filtering/classification, but they do not turn CASALS L1B into a formal multi-return point cloud.

Range-window note:
- The previous standalone range-window hypothesis notebook has been integrated into `notebooks/01b_l1b_sweep_waveform_matrix.ipynb` as a sweep-level sanity check.
...
- `notebooks/01b_l1b_sweep_waveform_matrix.ipynb`
- `notebooks/02_l1b_waveform_feature_extraction.ipynb`
- `notebooks/03_l1b_waveform_features_vs_refh_quality.ipynb`
